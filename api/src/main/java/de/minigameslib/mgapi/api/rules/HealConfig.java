 ...
 * @author mepeisen
 * 
 * @see BasicZoneRuleSets#Heal
 */
@ConfigurationValues(path = "core")
public enum HealConfig implements ConfigurationValueInterface
{
    
    /**
     * The heal cooldown in seconds
     */
    @ConfigurationInt(defaultValue = 30)
    @ConfigComment({"Heal cooldown in seconds"})
    @ValidateLMin(0)
    Cooldown,
    ...
    @ConfigComment({"Maximum heal ticks before starting cooldown"})
    @ValidateLMin(0)
    MaxHealTicks,
