...
import de.minigameslib.mgapi.api.rules.HealAtBeginConfig;

/**
 * @author mepeisen
 *
 */
public class HealAtBegin extends AbstractArenaRule implements HealAfterDeathRuleInterface
{
     ...
     */
    public HealAtBegin(ArenaRuleSetType type, ArenaInterface arena) throws McException
    {
        super(type, arena);
        this.runInCopiedContext(() -> {
            HealAtBeginConfig.HealPerTick.verifyConfig();
            this.healPerTick = HealAtBeginConfig.HealPerTick.getFloat();
            this.healTicks = HealAtBeginConfig.MaxHealTicks.getInt();
        });
    }
    ...
    public void setHealPerTick(float amount) throws McException
    {
        ...
        this.runInCopiedContext(() -> {
            HealAtBeginConfig.HealPerTick.setFloat(amount);
            try
            {
                HealAtBeginConfig.HealPerTick.verifyConfig();
                HealAtBeginConfig.HealPerTick.saveConfig();
            }
            catch (McException ex)
            {
                HealAtBeginConfig.HealPerTick.rollbackConfig();
                throw ex;
            }
        });
     ...
     */
    @McEventHandler
    public void onPlayerDies(ArenaPlayerDieEvent evt)
    {
        if (this.task != null)
        {
            this.task.playerDies(evt.getPlayer().getBukkitPlayer());
        }
    }
