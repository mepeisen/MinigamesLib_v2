 ...
 * @author mepeisen
 */
public class SpawnsStep implements WizardStepInterface<ArenaInterface>
{
     ...
     * @param arena
     */
    public SpawnsStep(ArenaInterface arena)
    {
        // does nothing
    }
    ...
    {
        final int maxPlayers = wizard.getStep(BasicMatchStep.class).getMaxPlayers();
        return new ClickGuiItem[][]{
            {
                new ClickGuiItem(ItemServiceInterface.instance().createItem(CommonItems.App_New), Messages.NewSpawn, (p, s, g) -> this.onNewSpawn(wizard, p, s, g), this.spawns.size() + 1, maxPlayers),
                null,
    ...
    private void setup(ArenaInterface arena) throws McException
    {
        ...
        {
            arena.createComponent(this.spawns.get(i), BasicComponentTypes.Spawn).setName("spawn" + (i + 1)); //$NON-NLS-1$
        }
    }
    ...
    private void checkSpawns(AbstractWizard<ArenaInterface> wizard) throws McException
    {
        ...
        {
            throw new McException(Messages.SpawnMissing, maxPlayers - this.spawns.size());
        }
        
        final BattleZoneStep battle = wizard.getStep(BattleZoneStep.class);
        battle.checkLocation(wizard);
        final Cuboid cub = battle.getCuboid();
        
        for (int i = 0; i < this.spawns.size(); i++)
        {
            if (!cub.containsLoc(this.spawns.get(i)))
            {
                throw new McException(SpawnComponent.Messages.NotWithinBattleZone, "spawn" + (i + 1)); //$NON-NLS-1$
            }
        }
    }
     ...
     * @author mepeisen
     */
    @LocalizedMessages(value = "admingui.wizard.arena_create.SpawnsStep")
    public enum Messages implements LocalizedMessageInterface
    {
         ...
         * help button
         */
        @LocalizedMessageList({"Create the spawns, at least one spawn for each player."})
         ...
         * spawn location.
         */
        @LocalizedMessage(defaultMessage = "Select spawn location")
         ...
         * spawn location.
         */
        @LocalizedMessageList("Use the pinion and click the block of spawn #%1$d.")
        @MessageComment({"spawn location"})
         ...
         * spawn missing.
         */
        @LocalizedMessage(defaultMessage = "At least %1$s spawns are missing")
        @MessageComment({"spawn missing"})
        SpawnMissing,
